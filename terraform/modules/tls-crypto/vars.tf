...
variable "tls_bucket" {
  type        = string
  description = "GCS Bucket to store resulting certificates and keys."
}
...
variable "tls_dns_names" {
  description = "List of DNS names added to the Vault server self-signed certificate. E.g vault.example.net"
  type        = list(string)
  default     = []
}
